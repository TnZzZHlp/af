...
pub async fn chat_completions(
    ...
    let whitelist = auth::fetch_model_whitelist(&state.pool, gateway_key_id).await?;
    if !whitelist.is_empty() && !whitelist.iter().any(|entry| entry == &model) {
        return Err(AppError::Forbidden);
    }

    let mut targets =
        routing::fetch_alias_target_details(&state.pool, &model, "openai_chat_completions")
            .await?;
    let target = targets
        .pop()
        .ok_or_else(|| AppError::BadRequest(format!("unknown model alias: {model}")))?;

    let mut provider_keys = routing::fetch_provider_keys(&state.pool, target.provider_id).await?;
    ...
    let request_body = serde_json::to_vec(&payload).map_err(|err| AppError::Internal(err.into()))?;
    let response = state
        .http_client
        .post(&url)
        .header(header::AUTHORIZATION, format!("Bearer {}", provider_key.key))
